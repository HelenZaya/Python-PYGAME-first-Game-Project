...
pygame.display.set_caption("Human Enemies + Horse Mount (E fix)")
clock = pygame.time.Clock()
font = pygame.font.SysFont("", 18)
pygame.mouse.set_visible(False)
pygame.event.set_grab(True)

# ---- HUD weapon icons (pygame-drawn) ----
def draw_weapon_icon(surface, x, y, kind, selected=False):
    """
    Draw a small weapon icon at (x, y) using pygame primitives.
    kind: "sword" or "bow"
    selected: if True, draw a highlight frame
    """
    import pygame, math
    icon = pygame.Surface((64, 64), pygame.SRCALPHA)
    pygame.draw.circle(icon, (30, 30, 40, 200), (32, 32), 30)
    if selected:
        pygame.draw.circle(icon, (255, 230, 120), (32, 32), 30, 3)
    else:
        pygame.draw.circle(icon, (180, 180, 200), (32, 32), 30, 1)
    if kind == "sword":
        # (removed sword HUD icon visuals)
        pass



    elif kind == "bow":
        # Bow icon: long & thick brown shaft with more pronounced inward-curved tips
        shaft_x = 32
        brown = (150, 100, 60)
        # main shaft (longer: y 6..58, thicker: 14px)
        pygame.draw.line(icon, brown, (shaft_x, 6), (shaft_x, 58), 14)
        # top inward curve (thicker: 10px, slightly deeper bend)
        pygame.draw.lines(icon, brown, False, [(shaft_x, 6), (shaft_x-6, 12), (shaft_x, 18)], 10)
        # bottom inward curve
        pygame.draw.lines(icon, brown, False, [(shaft_x, 58), (shaft_x-6, 52), (shaft_x, 46)], 10)
        # light edge accent
        pygame.draw.line(icon, (200, 160, 120), (shaft_x-4, 8), (shaft_x-4, 56), 2)
        ...
        def _draw_weapon_local(kind):
            ...
            if kind == "sword":
                ...
                max_off   = 10           # stronger curvature
                                # curved limbs: place pixels with inward curve near tips
                for yy in range(-half_h, half_h+1, step_y):
                    t = abs(yy) / half_h          # 0..1
                    # more rounded inward curve: starts near center, bends more at tips
                    off = int(max_off * (t**0.65))  # increase quicker toward tips
                    # left side pixels forming the limb curve
                    _px_rect(surf, brown, hx - off, hy + yy, 2, 2)
                # light accent along the inner edge
...
class Player(Humanoid):
    ...
    def update(self, plats, bullets, enemies, mouse_rel, time_s, keys, mounts):
        ...
        for e in enemies:
            if self.rect.colliderect(e.rect): self.take_damage(ENEMY_COLLIDE_TICK_DAMAGE)
        ...
        if self.is_enemy:
            if self.rect.colliderect(player.rect):
                if getattr(player, 'shield_active', False):
                    ...
                    player.take_damage(30); self.kill()
        else:
            hit = pygame.sprite.spritecollideany(self, enemies)
            if hit:
                hit.take_damage(30); self.kill()
        for t in plats:
